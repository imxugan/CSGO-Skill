                            ...
                            if (!handled) {
                                ...
                                col.insertOne(doc, {}, (err, result) => {
                                    ...
                                    skill.db('collection:Stats', (err, col) => {
                                        if (assistant.e(207, err)) { return } // This would suck
                                        col.insertOne(assistant.getStatTemplate(steamid), {}, (err, result) => {assistant.e(208, err)})
                                    })
                                    doc.success = true
                ...
                col.find({$or: [
                    ...
                    col.findOneAndUpdate({'steam_id': post.steamid}, ndoc, {}, (err, result) => {
                        ...
                        mailer.send({
                          ...
                          .catch(error => { assistant.e(306, error) })

                        skill.db('collection:Stats', (err, col) => {
                            if (assistant.e(304, err)) { return } // This would really suck
                            ndoc = { 'user': true }
                            assistant.getStatTemplate(null, true).forEach((k) => { ndoc['grand.'+k] = 0 })
                            ndoc = {$set: ndoc}
                            col.findOneAndUpdate({'steam_id': post.steamid}, ndoc, {}, (err, result) => {assistant.e(305, err)})
                        })
    ...
    router.get(['/profile/:id', '/user/:id'], (req, res) => {
        ...
        var sendDetails = function(profile) {
            ...
            if (!profile) {
                ...
                res.json({
                    success: true,
                    user: (profile.verified) ? true : false, // Incase verified isn't set
