        ...
        onTick: () => {
            doJobCheck('check-updates', interval, () => {
                queue.createJob('check-updates').run((error, job) => {
                    console.log(`Job ${job.id} created for 'check-updates'`)
                })
            })
        },
        onComplete: () => {
            console.warn(`Cron job 'check-updates' was stopped`)
        },
        startNow: true,
        ...
        onTick: () => {
            doJobCheck('update-stats', interval, () => {
                queue.createJob('update-stats').run((error, job) => {
                    console.log(`Job ${job.id} created for 'update-stats'`)
                })
            })
        },
        onComplete: () => {
            console.warn(`Cron job 'update-stats' was stopped`)
        },
        startNow: true,
        timeZone: 'Europe/London' // For UTC time
    })
