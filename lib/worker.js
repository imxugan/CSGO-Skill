...
queue.processJob('update-stats-single', (job, done) => {
    ...
    var steam_id = job.data.steam_id
    ...
    skill.db('collection:Players', (error, db, col) => {
        ...
        col.findOne({'steam_id': steam_id}).toArray((err, doc) => {
            if (err) {
                db.close()
                // Still nothing we can do
                return
            }
            if (doc.steam_id != steam_id) {
                db.close()
            ...
            var player = doc
            var timeLimit = +new Date() - 20 * 60 * 60 * 1000 // 20 hours to retry daily
            ...
            if (player.status.updateFailed.status == 'waiting' && player.status.updateFailed.time > timeLimit) {
                col.findOneAndUpdate({steam_id: player.steam_id}, {
                    $set: {
                        ...
                        'status.updateFailed.timeStarted': +new Date()
                    }
                }, (err, result) => {
                    if (err) {
                        // TODO
...
function updateStats(type, job, done) {
    if (type == 'all')
        ...
        console.log(`Processing Job ${job.id}: 'update-stats-single' for user ${job.data.steam_id}`)
    ...
    skill.db('collection:Stats', (error, db, col) => {
        ...
        if (type == 'all')
            ...
            search = {steam_id: job.data.steam_id}
        col.find(search).toArray((err, docs) => {
            ...
            var activeLimit = +new Date() - (30 * 24 * 60 * 60 * 1000),  // 30 days
                ...
                trackLimit = +new Date() - (48 * 60 * 60 * 1000)         // 48 hours
            ...
            for (i = 0; i < (playerInfo.total + 1); i++) { // Add MEGA back to total
                ...
                try {
                    ...
                    if (req.statusCode !== 200) {
                        playerInfo.failedRequests.push({steam_id: player.steam_id, response: req})
                        continue
                    }
                } catch (error) {
                    playerInfo.failedRequests.push({steam_id: player.steam_id, error: error})
                    continue
                }
                ...
                try {
                    ...
                    playerInfo.badRequests.push({steam_id: player.steam_id, error: error, response: req})
                    continue // Only try this once per user
                }
                
                if (stats.error) {
                    // Despite an error here, we may still have some stats returned.
                    playerInfo.badStats.push({steam_id: player.steam_id, error: stats.error})
                }
                ...
                try {
                                ...
                                if (player.current.now[stat] !== stats[stat]) {
                                    ...
                                    if (player.current.now[stat]) {
                                        ...
                                        if (g.includes(stat) && player.grand[stat]) {
                                            player.grand[stat] = player.grand[stat] + changes[stat]
                    ...
                    // We'll keep trying with this player
                    playerInfo.badChanges.push({steam_id: player.steam_id, error: error})
                }
                ...
                if (changed) {
                    ...
                    col.findOneAndUpdate({steam_id: player.steam_id}, {
                        ...
                        if (err) {
                            playerInfo.badUpdate.push({steam_id: player.steam_id, error: err})
                        } else {
            ...
            col.findOne({steam_id: 'mega'}, (err, doc) => {
                ...
                for (stat in mega) {
                    if (mega.hasOwnProperty(stat)) {
                        new_mega[stat] = BigNumber(doc.stats[stat] || 0).add(mega[stat]).toString()
                    }
                }
                col.findOneAndUpdate({steam_id:'mega'}, {$set:{'stats':new_mega}}, (err, result) => {
                    db.close()
                    if (err) {
                        var m
                        try {
            ...
            skill.db('collection:StatLogs', (err, db, col) => {
                ...
                var logDump = {
                    ...
                    'totalBadChanges': playerInfo.badChanges.length,
                    'badChanges': playerInfo.badChanges,
