...
queue.processJob('update-stats', (job, done) => {
    ...
    skill.db('collection:Stats', (error, db, col) => {
        if (error) {
            db.close()
            return ohCrap(error, 'connecting to MongoDB')
        }
        col.find({}).toArray((err, docs) => {
            if (err) {
                db.close()
                return ohCrap(error, 'getting profiles from Stats')
            }
            ...
            for (i = 0; i < totalPlayers; i++) {
                ...
                if (player.current.now.updated > trackLimit) {
                    ...
                    // Otherwise, just update the current so daily tracking can resume tomorrow
                    for (stat in stats) {
                        if (stats.hasOwnProperty(stat)) {
                            player.current.now[stat] = stats[stat]
                        }
            ...
            col.findOne({steam_id: 'mega'}, (err, doc) => {
                ...
                col.findOneAndUpdate({steam_id:'mega'}, {$set:{'stats':new_mega}}, (err, result) => {
                    ...
                    mailer.send({
                        ...
                        html: ejs.render(template.email.devmegastats, {'mega':mega, 'totalPlayers': totalPlayers})
                    }).then(() => { /* Nothing to do here */ })
                    .catch(error => { console.error(`Job ${job.id}: 'update-stats' sendMail to devteam failed: ${error}`) })
                })
            })
