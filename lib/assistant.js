...
exports.validateAccount = function(steamid) {
    ...
    if (!summary.success) {
        assistant.e(300, summary.error)
        return { 'success': false, 'reason': 'error code 300' }
    }

    if (summary.res.response.players[0].steamid !== steamid) {
        return { 'success': false, 'reason': 'validate-failed' }
    }

    var player = summary.res.response.players[0]

    if (player.communityvisibilitystate !== 3) {
        return { 'success': false, 'reason': 'private-account' }
    }

    // Check for ownership of CS: GO
    var games = exports.downloadOwnsCSGO(steamid)
    if (!games.success) {
        assistant.e(301, games.error)
        return { 'success': false, 'reason': 'error code 301' }
    }

    if (games.res.response.game_count !== 1) {
        return { 'success': false, 'reason': 'no-game' }
    }

    var playtime = games.res.response.games[0].playtime_forever
    if (playtime == 0) {
        return { 'success': false, 'reason': 'playtime-private' }
    } else if (playtime < 600) {
        return { 'success': false, 'reason': 'playtime' }
    }

    // Check for public stats
    var stats = exports.downloadStats(steamid)
    if (!stats.success) {
        if (stats.error.statusCode == 500) {
            return { 'success': false, 'reason': 'private-stats-or-down' }
        } else {
            return { 'success': false, 'reason': 'error code 302' }
        }
    }
...
exports.isPersona = function(persona) {
    try {
...
exports.getStatTemplate = function(steamid, isUser) {
    ...
    if (current.success) {
        ...
        exports.e(302, new Error(`Failed to retrieve stats for ${steamid}. "${current.error}"`))
        current = {}
    }
    var template = {
        'steam_id': steamid,
