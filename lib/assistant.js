...
var consoleResponse = function (output) {
...
var downloadOwnsCSGO = function (steamId) {
    try {
        var res = request('GET',
                          'http://api.steampowered.com/IPlayerService/GetOwnedGames/' +
                          `v0001/?key=${STEAMKEY}&input_json={%22steamid%22:%22${steamId}%22,` +
...
exports.downloadOwnsCSGO = downloadOwnsCSGO

var downloadSummaries = function (steamId) {
    try {
        var res = request('GET',
...
exports.downloadSummaries = downloadSummaries

var downloadStats = function (steamId) {
    try {
        var res = request('GET',
...
exports.downloadStats = downloadStats
...
var isValidToken = function (token, limit) {
    if (typeof token === 'object' &&
        ...
        var seconds = Math.abs(token.time - (Number(new Date()))) / 1000
        if (!limit)
            limit = 6 * 24 * 60 * 60 // default 6 day limit before resetting tokens
 ...
 * - steamId
 */
var validateAccount = function (steamId) {

    // Check for public settings
    var summary = downloadSummaries(steamId)
    if (!summary.success) {
        e(300, summary.error)
        return {
            'success': false,
    ...
    // Check for ownership of CS: GO
    var games = downloadOwnsCSGO(steamId)
    if (!games.success) {
        e(301, games.error)
        return {
            'success': false,
    ...
    if (games.res.response.game_count !== 1)
        return {
            'success': false,
            'reason': NO_GAME // NO LIFE
        }

    var playtime = games.res.response.games[0].playtime_forever
    if (playtime === 0)
        return {
            'success': false,
            'reason': PLAYTIME_PRIVATE
        }
    if (playtime < 600)
        return {
            'success': false,
    ...
    var stats = downloadStats(steamId)
    if (!stats.success) {
        if (stats.error.statusCode === 500)
            return {
                'success': false,
                'reason': PRIVATE_STATS_OR_DOWN
            }
...
var isPersona = function (persona) {
    try {
        var fursona = persona.toString()
        if (fursona.length < 3 || fursona.length > 35)
            return false
...
var fixUsername = function (username) {
    try {
...
var getStatTemplate = function (steamId, isUser) {
    if (typeof isUser !== 'boolean')
        isUser = false

    if (isUser)
        return [
            'knife',         'nades',         'fires',         'flash',
            'doms',          'revs',          'overkill',      'pistolwin',
            'donation',      'wins_cbble',    'wins_dust2',    'wins_inferno',
            'wins_nuke',     'wins_train',    'rnds_cbble',    'rnds_dust2',
            'rnds_inferno',  'rnds_nuke',     'rnds_train'
        ]

    var current = downloadStats(steamId)
    if (current.success) {
        current.res.updated = Number(new Date())
        current = {
            'now': current.res,
            'recent': current.res
        }
    } else {
        e(302, new Error(`Failed to retrieve stats for ${steamId}. "${current.error}"`))
        current = {}
    }
    var template = {
        'steam_id': steamId,
...
exports.getStatTemplate = getStatTemplate
...
class Timer {
    ...
    stop() {
        ...
        console.log(this.label + ((Number(new Date())) - this.start) + 'ms')
        this.label = null
        this.start = null
    }
}
