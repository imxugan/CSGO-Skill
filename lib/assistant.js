...
exports.getToken = function() {
    return {
        'time': +new Date(),
        'token': crypto.createHash('sha256').update(crypto.randomBytes(128)).digest('hex')
    }
}

// Global definition for a valid token
exports.isValidToken = function(token) {
    if (typeof token === 'object' && typeof token.value === 'string' && typeof token.time === 'number') {
        var days = Math.abs(token.time - (+new Date())) / 86400000 // milliseconds in a day
        var dayLimit = 6 // the day limit before resetting tokens
...
exports.getStatTemplate = function(steamid, isUser) {
    ...
    if (isUser) {
        return {
            'knife': 0,         'nades': 0,         'fires': 0,         'doms': 0,
            'revs': 0,          'overkill': 0,      'pistolwin': 0,     'donation': 0,
            'wins_cbble': 0,    'wins_dust2': 0,    'wins_inferno': 0,  'wins_nuke': 0,
            'wins_train': 0,    'rnds_cbble': 0,    'rnds_dust2': 0,    'rnds_inferno': 0,
            'rnds_nuke': 0,     'rnds_train': 0
        }
    }
...
//  Glock-18                                     P2000 & USP-S
    r.g0s  = c.total_shots_glock;                r.g2s  = c.total_shots_hpk2000
    r.g0h  = c.total_hits_glock;                 r.g2h  = c.total_hits_hpk2000
    r.g0k  = c.total_kills_glock;                r.g2k  = c.total_kills_hpk2000
//  Dual Berettas                                P250
